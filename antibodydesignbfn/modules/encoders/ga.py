...
from antibodydesignbfn.modules.common.geometry import global_to_local, local_to_global, normalize_vector, construct_3d_basis, angstrom_to_nm
from antibodydesignbfn.modules.common.layers import mask_zero, LayerNorm
from antibodydesignbfn.utils.protein.constants import BBHeavyAtom
...
class GABlock(nn.Module):
    ...
    def _pair_aggregation(self, alpha, z):
        ...
        # alpha: (N, L, L, n_heads)
        # z:     (N, L, L, C)
        # Output: (N, L, n_heads, C) -> reshape to (N, L, n_heads*C)
        
        # Original: feat_p2n = (alpha.unsqueeze(-1) * z.unsqueeze(-2)).sum(dim=2)
        # Optimized with einsum:
        feat_p2n = torch.einsum('nijh,nijc->nihc', alpha, z)
    ...
    def _node_aggregation(self, alpha, x):
        ...
        # Original: feat_node = (alpha.unsqueeze(-1) * value_l.unsqueeze(1)).sum(dim=2)
        # alpha: (N, L, L, n_heads) -> nijh
        # value_l: (N, L, n_heads, v_ch) -> njhc (source node j)
        # output: nihc
        feat_node = torch.einsum('nijh,njhc->nihc', alpha, value_l)
    ...
    def _spatial_aggregation(self, alpha, R, t, x):
        ...
        # Optimized:
        # alpha: nijh
        # value_points: njhpk (k=3)
        aggr_points = torch.einsum('nijh,njhpk->nihpk', alpha, value_points)
...
class GAEncoder(nn.Module):
    ...
    def forward(self, R, t, res_feat, pair_feat, mask):
        for i, block in enumerate(self.blocks):
            if self.training and res_feat.requires_grad:
                # Use gradient checkpointing to save memory
                res_feat = checkpoint(block, R, t, res_feat, pair_feat, mask)
            else:
