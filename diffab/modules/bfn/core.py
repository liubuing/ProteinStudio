...
from diffab.modules.bfn.sequence import CategoricalBFN
from diffab.modules.bfn.position import PositionBFN
from diffab.modules.bfn.orientation import OrientationBFN
from diffab.modules.bfn.sidechain import SidechainBFN
from diffab.modules.bfn.receiver import AntibodyBFN_Receiver
from diffab.modules.common.geometry import repr_6d_to_rotation_matrix, compute_fape
from diffab.modules.common.ot_align import LieOTAlign
...
class AntibodyBFN_Core(nn.Module):
    def __init__(self, res_feat_dim, pair_feat_dim, num_steps, eps_net_opt={}, position_mean=[0.0, 0.0, 0.0], position_scale=[10.0], loss_weight={}, ot_opt={}, beta=1.0, schedule='linear'):
        ...
        # Detect seq-only mode (no structure or confidence losses)
        ...
        seq_only = not has_structure_loss and not has_conf_loss
    ...
    def forward(self, batch):
        ...
        if 'af2_plddt' in batch and 'plddt' in self.loss_weight and self.loss_weight['plddt'] > 0:
            af2_plddt = batch['af2_plddt'][:, :pred_plddt.shape[1]]
            mask_plddt = mask_res[:, :pred_plddt.shape[1]]
        ...
        if 'af2_pae_matrix' in batch and 'pae' in self.loss_weight and self.loss_weight['pae'] > 0:
            af2_pae = batch['af2_pae_matrix'][:, :pred_pae.shape[1], :pred_pae.shape[2]]
            mask_row = mask_res[:, :pred_pae.shape[1]].unsqueeze(-1)
            mask_col = mask_res[:, :pred_pae.shape[2]].unsqueeze(1)
