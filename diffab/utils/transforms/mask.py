...
class MaskSingleCDR(object):
    ...
    def __init__(self, selection=None, augmentation=True):
        ...
        if selection is None:
            ...
            for s in selection:
                assert s in cdr_str_to_enum, f"Invalid CDR selection: {s}"
            self.selection = [cdr_str_to_enum[s] for s in selection]
        else:
            assert selection in cdr_str_to_enum, f"Invalid CDR selection: {selection}"
            self.selection = cdr_str_to_enum[selection]
    ...
    def perform_masking_(self, data, selection=None):
        ...
        if cdr_to_mask_flag.sum() == 0:
            # CDR not found in this chain
            return
        ...
        data['cdr_type'] = cdr_to_mask

    def __call__(self, structure):
        # 1. Identify all available CDRs in the current structure
        available_cdrs = []
        if structure['heavy'] is not None:
            available_cdrs.extend([
                (structure['heavy'], constants.CDR.H1),
                (structure['heavy'], constants.CDR.H2),
                (structure['heavy'], constants.CDR.H3),
            ])
        if structure['light'] is not None:
            available_cdrs.extend([
                (structure['light'], constants.CDR.L1),
                (structure['light'], constants.CDR.L2),
                (structure['light'], constants.CDR.L3),
            ])

        # 2. Filter available CDRs based on user selection
        candidates = []
        if self.selection is None:
            candidates = available_cdrs
        else:
            # Handle both single value and list
            target_selections = self.selection if isinstance(self.selection, (list, tuple)) else [self.selection]
            
            # Expand 'CDR3' alias if present
            expanded_targets = []
            for t in target_selections:
                if t == 'CDR3':
                    expanded_targets.append(constants.CDR.H3)
                    expanded_targets.append(constants.CDR.L3)
                else:
                    try:
                        expanded_targets.append(int(t))
                    except ValueError:
                        pass # Ignore invalid enum values

            # Intersect
            for chain_data, cdr_type in available_cdrs:
                if cdr_type in expanded_targets:
                    candidates.append((chain_data, cdr_type))

        # 3. Randomly select one from candidates
        if len(candidates) == 0:
            return structure # No matching CDRs found (e.g. Light chain only, but asked for H3)
        
        data_to_mask, sel = random.choice(candidates)
