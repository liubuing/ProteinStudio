...
def preprocess_antibody_structure(task):
    ...
    try:
        model = parser.get_structure(task['id'], pdb_path)[0]
        ...
        if 'antigen_id' in task and task['antigen_id'] is not None:
            ag_chain_ids = [task['antigen_id']]
        else:
...
class CustomDataset(Dataset):
    ...
    def __init__(self, structure_dir, heavy_id='H', light_id='L', transform=None, reset=False):
        super().__init__()
    ...
    def _preprocess_structures(self, pdb_list):
        ...
        for pdb_fname in pdb_list:
            ...
            tasks.append({
                'id': pdb_fname,
                'pdb_path': pdb_path,
                'heavy_id': getattr(self, 'heavy_id', 'H'),
                'light_id': getattr(self, 'light_id', 'L'),
            })
...
@register_dataset('custom')
def get_custom_dataset(cfg, transform):
    return CustomDataset(
        structure_dir = cfg.structure_dir,
        heavy_id = cfg.get('heavy_id', 'H'),
        light_id = cfg.get('light_id', 'L'),
        transform = transform,
    )
